        ...
        while True:
            ...
            if data_byte:
                ...
                if vac_inferior_feedback_bit == '1': # Se o feedback é '1', está LIGADO
                    ...
                    # Mantenha o estado PENDING_ON se a GPIO ainda está comandada como ATIVA,
                    # caso contrário, significa que foi desligado.
                    if current_gpio_output_states[GPIO_LINE_OFFSETS[2]] == gpiod.line.Value.ACTIVE and vac_inferior_internal_state == COMPONENT_STATUS[GPIO_LINE_OFFSETS[2]]["PENDING_ON"]:
                        pass # Continua PENDING_ON, pois o feedback '0' ainda não é o '1' esperado
                    else:
                        vac_inferior_internal_state = COMPONENT_STATUS[GPIO_LINE_OFFSETS[2]]["OFF"]
                ...
                if cilindro_feedback_bit == '1': # Se o feedback é '1', está AVANÇADO
                    ...
                    # Mantenha o estado PENDING_EXTEND/PENDING_RETRACT se a GPIO ainda está comandada,
                    # caso contrário, significa que foi retraído.
                    if current_gpio_output_states[GPIO_LINE_OFFSETS[3]] == gpiod.line.Value.ACTIVE and cilindro_internal_state == COMPONENT_STATUS[GPIO_LINE_OFFSETS[3]]["PENDING_EXTEND"]:
                        pass # Continua PENDING_EXTEND
                    elif current_gpio_output_states[GPIO_LINE_OFFSETS[3]] == gpiod.line.Value.INACTIVE and cilindro_internal_state == COMPONENT_STATUS[GPIO_LINE_OFFSETS[3]]["PENDING_RETRACT"]:
                        pass # Continua PENDING_RETRACT
                    else:
                        cilindro_internal_state = COMPONENT_STATUS[GPIO_LINE_OFFSETS[3]]["RETRACTED"]
                ...
                if vac_superior_feedback_bit == '1': # Se o feedback é '1', está LIGADO
                    ...
                    # CORREÇÃO AQUI: Mantenha o estado PENDING_ON se a GPIO ainda está comandada como ATIVA (HIGH),
                    # caso contrário, significa que foi desligado.
                    if current_gpio_output_states[GPIO_LINE_OFFSETS[4]] == gpiod.line.Value.ACTIVE and vac_superior_internal_state == COMPONENT_STATUS[GPIO_LINE_OFFSETS[4]]["PENDING_ON"]:
                        pass # Continua PENDING_ON, pois o feedback '0' ainda não é o '1' esperado
                    else:
                        vac_superior_internal_state = COMPONENT_STATUS[GPIO_LINE_OFFSETS[4]]["OFF"]
